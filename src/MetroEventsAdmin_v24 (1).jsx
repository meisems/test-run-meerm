...
const CrewView = ({role:userRole,accessCodes,staff,setStaff,currentUserId,addLog})=>{
  ...
  const saveStaff=async()=>{
    ...
    if(editStaffId){
      ...
      // Supabase returns the user on data.user or data.session.user depending on version/config.
      const newUser=data.user||data.session?.user;
      if(!newUser){setFormErr('Account created but user session was not returned. Please refresh and try again.');return;}
