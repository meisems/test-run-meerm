// ══════════════════════════════════════════════════════════════════════
//  METRO EVENTS — ADMINISTRATIVE COMMAND CENTER  v2.2
...
const CHECKLIST_INIT = {
  'Pre-Production':[
    {id:'PP-001',label:'Confirm event date and venue with client',done:false,photoUrl:null},
    {id:'PP-002',label:'Send and collect signed contract',done:false,photoUrl:null},
    {id:'PP-003',label:'Receive down payment',done:false,photoUrl:null},
    {id:'PP-004',label:'Finalize event theme and color palette',done:false,photoUrl:null},
    {id:'PP-005',label:'Assign lead coordinator',done:false,photoUrl:null},
  ],
  'Fabrication':[
    {id:'FA-001',label:'Create backdrop design mock-up',done:false,photoUrl:null},
    {id:'FA-002',label:'Order fabrication materials',done:false,photoUrl:null},
    {id:'FA-003',label:'Build and paint backdrop panels',done:false,photoUrl:null},
    {id:'FA-004',label:'Fabricate table centerpieces',done:false,photoUrl:null},
  ],
  'Supplier':[
    {id:'SU-001',label:'Book catering supplier',done:false,photoUrl:null},
    {id:'SU-002',label:'Confirm florals order',done:false,photoUrl:null},
    {id:'SU-003',label:'Coordinate with sound and lights team',done:false,photoUrl:null},
    {id:'SU-004',label:'Collect supplier invoices',done:false,photoUrl:null},
  ],
  'Load-in':[
    {id:'LI-001',label:'Load all decor items to truck',done:false,photoUrl:null},
    {id:'LI-002',label:'Arrive at venue 4 hours before event',done:false,photoUrl:null},
    {id:'LI-003',label:'Set up backdrop and stage',done:false,photoUrl:null},
    {id:'LI-004',label:'Arrange table centerpieces and linens',done:false,photoUrl:null},
    {id:'LI-005',label:'Sound check and lighting test',done:false,photoUrl:null},
  ],
  'Load-out':[
    {id:'LO-001',label:'Dismantle backdrop and stage elements',done:false,photoUrl:null},
    {id:'LO-002',label:'Pack and inventory all rental items',done:false,photoUrl:null},
    {id:'LO-003',label:'Return items to warehouse',done:false,photoUrl:null},
    {id:'LO-004',label:'File post-event report',done:false,photoUrl:null},
  ],
};
    ...
    <Modal title={`Attach Reference Photo`} onClose={onClose}>
      ...
      <div
        ...
        style={{
          ...
          textAlign:'center',
          background:dragging?'var(--gold-faint)':preview?'var(--overlay)':'var(--overlay)',
        ...
        onMouseEnter={e=>{if(!preview){e.currentTarget.style.borderColor='var(--gold)';e.currentTarget.style.background='var(--gold-faint)';}}}
        onMouseLeave={e=>{if(!preview){e.currentTarget.style.borderColor='var(--border-s)';e.currentTarget.style.background='var(--overlay)';}}}
      >
        <input
          ref={fileRef}
          type="file"
      ...
      <div style={{display:'flex',gap:9,justifyContent:'flex-end',alignItems:'center'}}>
        ...
        {!preview&&(
          <GoldBtn onClick={()=>fileRef.current?.click()}>
...
══════════════════════════════════════════════════════ */
const CRMView = ({events,setEvents,role})=>{
...
══════════════════════════════════════════════════════ */
const ChecklistView = ({role, events=[]})=>{
  ...
  const addTask = ()=>{
    ...
    setItems(prev=>({...prev,[cat]:[...prev[cat],{id:newId,label:newLabel.trim(),done:false,photoUrl:null}]}));
    setNewLabel('');
    setShowAdd(false);
  };
...
══════════════════════════════════════════════════════ */
const CrewView = ({role:userRole,accessCodes,staff,setStaff,currentUserId})=>{
      ...
      {checkoutItem&&(
        <Modal title={`Check Out — ${checkoutItem.id}`} onClose={()=>setCheckoutItem(null)} wide>
          ...
          <div className="g-2" style={{gap:14}}>
            <div>
              <Field label="Assign to Event" value={coForm.event} onChange={v=>setCoForm({...coForm,event:v})} options={EVENTS_INIT.filter(e=>e.stage!=='Done').map(e=>({value:e.id,label:`${e.id} — ${e.client}`}))}/>
...
══════════════════════════════════════════════════════ */
const AuditView = ({role,accessCodes,setAccessCodes})=>{
...
══════════════════════════════════════════════════════ */
const AuthScreen = ({onLogin,accessCodes,staff,adminPassword})=>{
        ...
        {step===1&&(
          <div style={{animation:'fadeUp .3s ease'}}>
            ...
            <div style={{marginTop:16,padding:'10px 14px',background:'var(--overlay)',borderRadius:'var(--r-md)',fontSize:11,color:'var(--tm)',lineHeight:1.9}}>
              <strong style={{color:'var(--ts)'}}>Demo usernames:</strong> admin · mdelacruz · jreyes · asantos<br/>
...
══════════════════════════════════════════════════════ */
const ChangePasswordModal = ({user,staff,setStaff,adminPassword,setAdminPassword,onClose})=>{
  ...
  const submit=()=>{
    ...
    if(user.role==='admin'){
      if(curPw!==adminPassword){setErr('Current password is incorrect.');return;}
      setAdminPassword(newPw);
    } else {
      const staffRecord=staff.find(s=>s.id===user.staffId);
      if(!staffRecord){setErr('Staff record not found. Contact your administrator.');return;}
      if(curPw!==staffRecord.password){setErr('Current password is incorrect.');return;}
    ...
    <Modal title="Change Password" onClose={onClose}>
      {success?(
        <div style={{textAlign:'center',padding:'28px 20px'}}>
          ...
          <div style={{fontSize:15,fontWeight:600,color:'var(--tp)',marginBottom:6}}>Password Updated</div>
          <div style={{fontSize:13,color:'var(--ts)'}}>Your new password is now active. This dialog will close automatically.</div>
        </div>
      ):(
        <>
          <div style={{padding:'10px 14px',background:'var(--navy)',borderRadius:'var(--r-md)',marginBottom:18,display:'flex',gap:10,alignItems:'center'}}>
            ...
            <div>
              ...
              <div style={{fontSize:11,color:'var(--sidebar-text-dim)',marginTop:1}}>Changing password for <span style={{color:'var(--gold)'}}>{user.name}</span> · <span style={{fontFamily:"'DM Mono',monospace",fontSize:10}}>@{user.username}</span></div>
            </div>
          </div>
          ...
          <div style={{display:'flex',gap:10,justifyContent:'flex-end',marginTop:6}}>
            <GhostBtn onClick={onClose}>Cancel</GhostBtn>
            <GoldBtn onClick={submit} disabled={!curPw||!newPw||!confPw}><Lock size={13}/>Update Password</GoldBtn>
          </div>
        </>
      )}
    </Modal>
...
/* ══════════════════════════════════════════════════════
   ADMIN SHELL
══════════════════════════════════════════════════════ */
const AdminShell = ({user,onLogout,accessCodes,setAccessCodes,staff,setStaff,adminPassword,setAdminPassword})=>{
  ...
  const content={
    ...
    crm:<CRMView events={events} setEvents={setEvents} role={user.role}/>,
    checklist:<ChecklistView role={user.role} events={events}/>,
    crew:<CrewView role={user.role} accessCodes={accessCodes} staff={staff} setStaff={setStaff} currentUserId={user.staffId}/>,
    warehouse:<WarehouseView role={user.role}/>,
    supplier:<SupplierView role={user.role}/>,
    quotation:<QuotationView role={user.role}/>,
    audit:<AuditView role={user.role} accessCodes={accessCodes} setAccessCodes={setAccessCodes}/>,
  };
...
export default function AdminApp(){
  ...
  const handleLogin=u=>{ setUser(u); setView('shell'); };
